...
def detail_client(request, pk):
    ...
    ventes = client.ventes_liees.filter(statut='completee').order_by('-created_at')[:20]
...
def api_clients(request):
    ...
    if q:
        clients = clients.filter(Q(nom__icontains=q) | Q(telephone__icontains=q))
    ...
    data = [{'id': c.id, 'nom': c.nom, 'telephone': c.telephone} for c in clients[:limit]]
